...
export default function AmenityIndex() {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      /* The fixed bar covers the top of the section; the word has to start
         below it or its first letter is behind the bar. */
      const nav = parseFloat(
        getComputedStyle(document.documentElement).getPropertyValue('--nav-h'),
      ) || 0;
      const navPx = nav <= 10 ? nav * 16 : nav;
      if (r.width && r.height) {
        setBox({ w: Math.round(r.width), h: Math.round(r.height), top: Math.round(navPx + 16) });
      }
    };
    measure();
          ...
          <defs>
            <mask id="amenity-knockout" maskUnits="userSpaceOnUse" x="0" y="0" width={box.w} height={box.h}>
              ...
              {/* Baseline on the word's right edge, so the bottoms of the
                  ...
                  begins. The run is measured against the height MINUS the
                  fixed nav and centred in what is left — against the full
                  height the word ran up under the bar. */}
              <text
                x={box.w}
                y={midY}
