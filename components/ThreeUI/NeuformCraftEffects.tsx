...
import { NEXUS_UNIFIED_FLOW_SOURCE } from './nexus-unified-flow';

/**
 * ThreeUI NeuformCraftEffects, vendored from bundle `halftone-flow`
 * (src/shaders/neuform-isolated/NeuformCraftEffects.tsx, SHA-256
 * 0a1680c3c119dba8c61d946322afa0b64d36dfd80956fb5e7c3fd017d7bfa450).
 *
 * Kept as authored: the EFFECTS shape, clamp(), buildFocusedDocument() and its
 * injected focus style and isolate script, the prop contract and its defaults,
 * and the iframe — including `sandbox="allow-scripts"` and the fact that hue /
 * saturation / brightness are a CSS filter on the frame rather than shader
 * uniforms.
 *
 * Trimmed to the ONE effect this project vendored. The authored file imports
 * seven sibling .html sources through Vite's `?raw`; this project is Next, has
 * no `?raw` loader, and needs one of the seven. The other six definitions and
 * their two presentation adapters (animatedTypographySource,
 * wovenClothLabelSource) are dead weight here and are not carried.
 */
...
const EFFECTS = {
  nexusUnifiedFlow: {
    ...
    source: NEXUS_UNIFIED_FLOW_SOURCE,
    background: '#000000',
